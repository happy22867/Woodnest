...
const Home = () => {
  ...
  fetch("/api/logout", {
    method: "POST",
    credentials: "include", // agar cookie me refresh token hai
  }).then(res => res.json())
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "40px" }}>
              <Link to="#" style={navLink}>Locations</Link>
              <Link to="#" style={navLink}>Rooms</Link>
              <Link to="#" style={navLink}>Experiences</Link>
              <Link to="#" style={navLink}>Contact</Link>
              <button onClick={handleLogout} style={{ padding: "12px 28px", background: "#ff4d4d",  color: "#fff", border: "none",  borderRadius: "10px", fontWeight: 700,fontSize: "17px", cursor: "pointer",}}> Logout</button>
              <button style={{
              ...
              <div style={{ display: "flex", gap: "10px", marginBottom: "20px" }}>
                {[{ label: "Check-in", val: "After 2:00 PM" }, { label: "Check-out", val: "Until 12:00 PM" }].map((c) => (
                  <div key={c.label} style={infoBox}>
                    <div style={{ fontSize: "12px", color: "rgba(255, 255, 255, 0.747)", marginBottom: "5px" }}>{c.label}</div>
